...
// Test optimized uint64 field operations against the original big.Int methods
func TestFieldOptimizedOperations(t *testing.T) {
...
func BenchmarkFieldAddRegular(b *testing.B) {
	a := One()
	x := One()
	result := Zero()
...
func BenchmarkFieldAddOptimized(b *testing.B) {
	a := One()
	x := One()
	result := Zero()
...
func BenchmarkFieldMulRegular(b *testing.B) {
	a := One()
	x := One()
...
func BenchmarkFieldMulOptimized(b *testing.B) {
	a := One()
	x := One()
	// Set to non-trivial values
	aBytes := []byte{
